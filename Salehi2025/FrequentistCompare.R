...
# Run the simulations across various sample sizes and repetitions
sample_sizes <- c(30, 80, 150, 250, 500)
n_reps <- 5
...
results_shannon <- expand.grid(sample_size = sample_sizes, rep = 1:n_reps) %>%
  pmap_dfr(function(sample_size, rep) {
    message(glue::glue("Running Shannon model: sample_size = {sample_size}, repetition = {rep}"))
    simulate_shannon_once(adult_metadata, sample_size = sample_size) %>%
      mutate(rep = rep)
  })

# Save the full simulation output for further analysis or plotting
