...
- `ComparisonModels.R` – comparison of log-normal vs. log-transformed normal models (computationally intensive)

- `StratifiedModel.R` – stratified model fitting by subgroups such as income group and gender (computationally intensive)

- `HierarchicalModel.R` – hierarchical modeling with nested random effects (computationally intensive)

- `FrequentistCompare.R` – simulation-based comparison of Bayesian and frequentist models (computationally intensive)
...
Model fitting is computationally intensive and has been performed in a separate R script (`ComparisonModels.R`). Please ensure that this script is available in your working directory.
...
full_summary_gaussian <- lapply(names(fit_list_gaussian), function(r) {  
  lapply(incomes, function(ic) {
    ...
    # Extract posterior summaries for fixed effects (only coefficients for fixed effects)
    my_summary <- posterior_summary(fit_list_gaussian[[r]][[ic+1]]) %>%
      .[grep("^b_", rownames(.)), ] %>%                            
      as.data.frame() %>%
      rownames_to_column(var = "Feature") %>%
      mutate(
...
full_summary_lognormal <- lapply(names(fit_list_lognormal), function(r) {
  lapply(incomes, function(ic) {
    ...
    my_summary <- posterior_summary(fit_list_lognormal[[r]][[ic+1]]) %>%
      .[grep("^b_", rownames(.)), ] %>%                            
      as.data.frame() %>%
      rownames_to_column(var = "Feature") %>%
      mutate(
...
Model fitting is computationally intensive and has been performed in a separate R script (`StratifiedModel.R`). Please ensure that this script is available in your working directory. The models have been saved as `.rds` files and are loaded here to enable downstream analysis.
...
Model fitting in this case is computationally intensive and has been performed in a separate R script (`HierarchicalModel.R`). Please ensure that this script is available in your working directory. The models have been saved as `.rds` files and are loaded here to enable downstream analysis.
...
# Order predictors
age_order <- c("Oldest Adult", "Older Adult", "Young Adult", "Teenager", "Children", "Toddler", "Infant")

full_summary_hier$Predictor <- factor(full_summary_hier$Predictor, levels = age_order)
```
...
Model fitting and repetitions are computationally intensive and have been performed in a separate R script (`FrequentistCompare.R`). Please ensure that this script is available in your working directory. The model outputs have been saved as `.rds` files and are loaded here for downstream analysis.
